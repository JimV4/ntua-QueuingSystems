...
while transitions >= 0
  ...
  if mod(transitions,1000) == 0 % check for convergence every 1000 transitions steps
    ...
    if abs(mean_clients - previous_mean_clients) < 0.000001 || transitions > 500000 % convergence test
      break;
    endif
  ...
  if current_state == 0 || random_number < threshold % arrival
    ...
    try % to catch the exception if variable arrivals(i) is undefined. Required only for systems with infinite capacity.
